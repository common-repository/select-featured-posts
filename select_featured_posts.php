...
Description: A plugin which allows you to select/unselect posts and updates the same using Ajax.This allows you to mark the selected post as featured post and hence enables editors, publishers, bloggers etc to feature relavent content on the site.
Author: Kapil Chugh
Author URI: http://kapilchugh.wordpress.com/
Plugin URI: http://www.mediologysoftware.com/wordpress-plugins
Version: 0.1
*/
  global $wpdb;
      ...
      function add_featured_post(post_id) {
        if (!isNaN(post_id)) {
          if (!jQuery('#' + post_id).is(':checked')) {
            var confirmation = confirm('Are you sure you want to unfeatured this post');
            if (confirmation) {
              var delete_stat  = 1;
            } else {
            jQuery('#' + post_id).attr('checked', true);
              return false;
            }
          }
          jQuery.ajax({
          ...
          alert('Please select a valid post id');
        }
      }
      -->
      </script> <?php
  ...
  function add_featured_post_id () {
    ...
    if ($success) {
      ...
      echo 'There was some error while updating.Please try again';
    }
    exit;
  }
  ...
  class WP_Widget_Featured_Posts extends WP_Widget {
    ...
    function form( $instance ) {
      ...
      <p>
        <label>
          <?php _e( 'Category' ); ?>:
          <?php wp_dropdown_categories( array( 'show_option_all' => 'Select a Category', 'name' => $this->get_field_name("cat"), 'selected' => $instance["cat"] ) ); ?>
        </label>
      </p> <?php
    }
  }
